...
import * as testSuite from '../matrixArray'

describe('MatrixArray test suite', () => {
    it('should create a MatrixArray and return the right values', () => {
        const array: testSuite.MatrixArray = new testSuite.MatrixArray([[1, 2, 3], [4, 5, 6]])
        ...
        assert.deepEqual([[1, 2, 3], [4, 5, 6]], array.compact())
        assert.deepEqual(24, array.size())

        array.set([7, 7, 7])
        assert.deepEqual([[7, 7, 7], [4, 5, 6]], array.compact())
        array.set([8, 8, 8], 1)
        assert.deepEqual([[7, 7, 7], [8, 8, 8]], array.compact())
    })
})
